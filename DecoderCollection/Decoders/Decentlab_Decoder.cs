...
namespace SensorDecoderModule.Classes
{
    ...
    internal static partial class LoraDecoders
    {
        ...
        public static string DecentLab_DLPR26_Sensor_Decoder(string devEUI, byte[] payload, byte fport)
        {
            var decodedDict = Decode(payload);
            var decoded = new JObject();

            foreach (var key in decodedDict.Keys)
            {
                decoded.Add(key, decodedDict[key].Item1);
            }
        ...
        private static readonly List<Sensor> SENSORS = new List<Sensor>() {
            new Sensor(2, new List<SensorValue>() {
                new SensorValue("Pressure", "bar", x => (x[0] - 16384) / 32768 * ((1.0) - (0.0)) + (0.0)),
                new SensorValue("Temperature", "°C", x => (x[1] - 384) * 0.003125 - 50)
            }),
            new Sensor(1, new List<SensorValue>() {
                new SensorValue("BatteryVoltage", "V", x => x[0] / 1000)
            })
        };
        ...
        private static Dictionary<string, Tuple<double, string>> Decode(byte[] msg)
        {
            return Decode(new MemoryStream(msg));
        }

        private static Dictionary<string, Tuple<double, string>> Decode(String msg)
        {
        ...
        private static Dictionary<string, Tuple<double, string>> Decode(Stream msg)
        {
            ...
            result["ProtocolVersion"] = new Tuple<double, string>(version, null);

            var deviceId = ReadInt(msg);
            result["DeviceID"] = new Tuple<double, string>(deviceId, null);
